	...
	Deferred: function( func ) {
		...
		jQuery.each( tuples, function( i, tuple ) {
			...
			if ( stateString ) {
				...
				// 禁用对立的那条队列
				// 注意 0^1 = 1   1^1 = 0
				// 第一个回调函数用于处理Deferred的状态（已成功或已失败）
				// 第二个回调函数用于让另一个列表失效。
				// 这是因为当发布者发布内容的动作已经成功了，它就不可能再触发失败的状态，因此让失败列表失效
				// 防止给正在订阅了失败状态的函数被执行。反之亦然。
				// 然后锁定notify列表。锁定notify只是让其的progressList的fire方法（即后面不能使用notify和notifyWith再进行推送内容了）不能被调用了。.
